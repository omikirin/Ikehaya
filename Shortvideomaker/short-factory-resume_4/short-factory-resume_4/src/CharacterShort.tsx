...
const HookBackground: React.FC<{ bg: HookBg; theme: Theme }> = ({ bg, theme }) => {
  if (bg === 'washi') {
    const scallops = (y: number, flip: boolean) => {
      let d = `M -60 ${y}`;
      for (let i = 0; i < 7; i++) d += ` q 90 ${flip ? 76 : -76} 180 0`;
      return d;
    };
    return (
      <AbsoluteFill style={{ background: '#efe6d2' }}>
        <svg width={1080} height={1920} style={{ position: 'absolute', inset: 0 }}>
          <g fill="none" stroke="#b8933f" strokeWidth={6} opacity={0.55}>
            <path d={scallops(210, false)} />
            <path d={scallops(300, false)} strokeWidth={3} opacity={0.7} />
            <path d={scallops(1660, true)} />
            <path d={scallops(1570, true)} strokeWidth={3} opacity={0.7} />
          </g>
        </svg>
      </AbsoluteFill>
    );
...
export const CharacterShort: React.FC<{
  title: string;
  audioSrc: string;
    ...
    >
      ...
      {scene.type === 'split' && scene.src && scene.srcB ? (
        <AbsoluteFill style={{ transform: `scale(${punch})`, background: BG }}>
          ...
          <svg width={1080} height={1920} style={{ position: 'absolute', inset: 0 }}>
            <line x1={0} y1={1920 * 0.64} x2={1080} y2={1920 * 0.4} stroke={ACCENT} strokeWidth={10} />
          </svg>
          {scene.labelA ? (
            <div
              style={{
      ...
      {scene.type === 'manga' && scene.src ? (
        <AbsoluteFill style={{ transform: `scale(${punch})` }}>
          ...
          >
            <svg width={940} height={1180} style={{ position: 'absolute', inset: 0 }}>
      ...
      {scene.type === 'typo' && scene.text ? (
        ...
        >
          <div
            style={{
              fontSize: scene.textPos === 'top' ? 100 : scene.textPos === 'middle' ? 92 : 124,
              fontWeight: 800,
              color: hookInk,
              textAlign: 'center',
      ...
      {isAvatarScene ? (
        <div
          style={{
            ...
            transformOrigin: `540px ${Math.round(CHAR_TOP + DISP_H * 0.54)}px`,
          }}
        >
          {/* 接地影: 浮くほど小さく薄く */}
          <div
